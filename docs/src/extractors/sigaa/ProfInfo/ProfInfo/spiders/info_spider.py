...
class InfoSpiderSpider(scrapy.Spider):
    ...
    def parse(self, response: scrapy.http.Response) -> Generator[scrapy.Request, None, None]:
        ...
        def extrair_formacao_academica(seletor_id: str) -> Dict[str, str]:
            """
            Extrai e mapeia a formação acadêmica do docente.
            
            Itera sobre os elementos `<dt>` e `<dd>` dentro da lista de definição `<dl>`.
            Verifica o nome do campo e atribui o valor correspondente à chave correta.
            
            Args:
                seletor_id (str): Seletor CSS (ID) do container da formação (ex: '#formacao-academica').
                
            Returns:
                Dict[str, str]: Dicionário padronizado contendo os níveis de graduação.
            """
            dados = {
                "graduacao": "",
                "especializacao": "",
            ...
            blocos = response.css(f"{seletor_id} dl dt")
            for dt in blocos:
                nome_campo = dt.xpath("string(.)").get().strip().lower().replace(":", "")
                dd = dt.xpath("following-sibling::dd[1]")
                if dd:
                    valor = dd.xpath(".//text()").get(default="").strip() or \
                            dd.xpath(".//a/text()").get(default="").strip() or \
                            dd.xpath(".//i/text()").get(default="").strip() or ""
                    
                    if "graduação" in nome_campo: dados["graduacao"] = valor
                    elif "especialização" in nome_campo: dados["especializacao"] = valor
                    elif "mestrado" in nome_campo: dados["mestrado"] = valor
                    elif "doutorado" in nome_campo: dados["doutorado"] = valor
                    elif "pós-doutorado" in nome_campo or "pos-doutorado" in nome_campo: dados["pos-doutorado"] = valor
            return dados
        ...
        portal: Dict[str, Any] = {
            ...
            "formacao_academica": extrair_formacao_academica("#formacao-academica")
        }
